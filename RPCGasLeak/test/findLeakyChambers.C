...
void findLeakyChambers()
{
      ...
      iter != rpcPressureMap.end(); ++iter )
  {
    ...
    if ( true )
    {
      ...
      TFitResultPtr r1 = grpLeak->Fit("fLinear", "QSEM" , "", fitMin1, fitMax1);
      TFitResultPtr r2 = grpLeak->Fit("fLinear", "+QSEM", "", fitMin2, fitMax2);
      TFitResultPtr r3 = grpLeak->Fit("fLinear", "+QSEM", "", fitMin3, fitMax3);
      ...
      grpLeakRate1->SetPointError(nGrpLeakRate, 0.5, 0.5, -600*errDn1, 600*errUp1);
      grpLeakRate2->SetPointError(nGrpLeakRate, 0.5, 0.5, -600*errDn2, 600*errUp2);
      grpLeakRate3->SetPointError(nGrpLeakRate, 0.5, 0.5, -600*errDn3, 600*errUp3);

      hLeakRate1->Fill(TMath::Min(maxLeakRate-1e-9,-slope1*600));
      hLeakRate2->Fill(TMath::Min(maxLeakRate-1e-9,-slope2*600));
      hLeakRate3->Fill(TMath::Min(maxLeakRate-1e-9,-slope3*600));
    }

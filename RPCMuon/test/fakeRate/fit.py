...
def fit(hA, hB, c = None):
    ...
    mass = RooRealVar("vertexMass", "mass", massMin, massMax, "GeV/c^{2}")
    ...
    ws.factory("Voigtian::sigA(vertexMass, m0, w0[1e-2, 1e-5, 1e-1], sigmaA[1e-2, 1e-3, 1e-1])")
    ws.factory("Voigtian::sigB(vertexMass, m0, w0, sigmaA)")
    #ws.factory("Voigtian::sigB(vertexMass, m0, w0, sigmaB[1e-2, 1e-3, 1e+0])")
    ws.factory("Chebychev::bkgA(vertexMass, {p0A[0, -10, 10], p1A[0, -10, 10]})")
    ws.factory("Chebychev::bkgB(vertexMass, {p0B[0, -10, 10], p1B[0, -10, 10]})")

...
process.source = cms.Source("PoolSource", 
         ...
         #'file:/afs/cern.ch/user/m/mskim/workspace/public/rpc/02827972-BCAE-E111-85B0-001EC9D82BC3.root',
         #'file:/afs/cern.ch/user/m/mskim/workspace/public/rpc/043392CE-8BAD-E111-950C-001EC9D80781.root',
         '/store/data/Run2012D/SingleMu/RECO/PromptReco-v1/000/203/777/24F0F702-110B-E211-99C9-001D09F248F8.root',
    )
)
##for line in open('Run2012B-ZMu-PromptSkim-v1_n60.txt').readlines():
#for line in open('Run2012B-PromptReco-v1_12.txt').readlines():
#    line = line.strip("'\", \n")
...
process.promptMuons = cms.EDFilter("PromptMuonSelector",
    ...
    maxDz = cms.untracked.double(maxDz),
    beamSpot = cms.InputTag("offlineBeamSpot"),
    primaryVertex = cms.InputTag("offlinePrimaryVertices"),
)
process.promptMuonsNoRPC = process.promptMuons.clone(src = cms.InputTag("muonsNoRPC"))

# For Muon Isolation, go to https://twiki.cern.ch/twiki/bin/view/CMS/MuonIsolation#Suggested_cuts

process.tightMuons = cms.EDFilter("MuonRefSelector",
                                src = cms.InputTag("promptMuons"),
                                cut = cms.string("isGlobalMuon && isPFMuon && isolationR03().sumPt<0.05*pt" #--updated by Minsuk on Aug 3, 2012
                                                 "&& pt > 20 && abs(eta) < 2.4"
...
process.probeMuons = cms.EDFilter("MuonRefSelector",
    ...
    cut = cms.string("isRPCMuon && numberOfMatchedStations('RPCHitAndTrackArbitration') > 0"
                     "&& abs(innerTrack().dxy)<2.0 && pt > 20 && abs(eta) < 1.8"
...
process.MediumTightMuons = cms.EDFilter("MuonRefSelector",
                                  ...
                                  cut = cms.string("isGlobalMuon"
                                                   ...
                                                   #"&& globalTrack().hitPattern().numberOfValidMuonHits()>0"
                                                   "&& innerTrack().hitPattern().numberOfValidPixelHits() > 0"
                                                   ##"&& track().hitPattern().numberOfValidTrackerHits() > 10 && innerTrack().numberOfValidHits()>10"
                                                   "&& track().hitPattern().trackerLayersWithMeasurement() > 5"
                                                   ), 
                                  )
...
process.MediumTightMuonsRPCMu = cms.EDFilter("MuonRefSelector",
                                       src = cms.InputTag("promptMuons"),
                                       #src = cms.InputTag("muons"),
                                       cut = cms.string("((isGlobalMuon && globalTrack().normalizedChi2()<10.0) || (isRPCMuon && numberOfMatchedStations('RPCHitAndTrackArbitration') > 1))"
                                                        #"&& numberOfMatchedStations > 1"
                                                        "&& abs(innerTrack().dxy)<2.0 && pt > 0 && abs(eta) < 2.4"
                                                        #"&& globalTrack().normalizedChi2()<10.0"
                                                        #"&& globalTrack().hitPattern().numberOfValidMuonHits()>0"
...
process.TightMuonsRPCMu = cms.EDFilter("MuonRefSelector",
                                       src = cms.InputTag("promptMuons"),
                                       #src = cms.InputTag("muons"),
                                       cut = cms.string("((isGlobalMuon && globalTrack().normalizedChi2()<10.0 && numberOfMatchedStations > 1 && globalTrack().hitPattern().numberOfValidMuonHits()>0) || (isRPCMuon && numberOfMatchedStations('RPCHitAndTrackArbitration') > 1 && numberOfMatchedRPCLayers('RPCHitAndTrackArbitration') > 2))"
                                                        #"&& numberOfMatchedStations > 1"
                                                        "&& abs(innerTrack().dxy)<2.0 && pt > 0 && abs(eta) < 2.4"
                                                        #"&& globalTrack().normalizedChi2()<10.0"
                                                        #"&& globalTrack().hitPattern().numberOfValidMuonHits()>0"
...
process.tkToMediumTightMuonsRPCMu = cms.EDProducer("MatcherUsingTracks",
                                     src     = cms.InputTag("promptTrackCands"), # all tracks are available for matching
                                     matched = cms.InputTag("MediumTightMuonsRPCMu"), # to all global muons
                                     algorithm = cms.string("byDirectComparison"), # check that they
                                     srcTrack = cms.string("tracker"),             # have the same
                                     srcState = cms.string("atVertex"),            # tracker track
                                     matchedTrack = cms.string("tracker"),         # can't check ref
...
process.tkToTightMuonsRPCMu = cms.EDProducer("MatcherUsingTracks",
                                     src     = cms.InputTag("promptTrackCands"), # all tracks are available for matching
                                     matched = cms.InputTag("TightMuonsRPCMu"), # to all global muons
                                     algorithm = cms.string("byDirectComparison"), # check that they
                                     srcTrack = cms.string("tracker"),             # have the same
                                     srcState = cms.string("atVertex"),            # tracker track
                                     matchedTrack = cms.string("tracker"),         # can't check ref
...
process.passingMediumTightMuonsRPCMu = cms.EDProducer("MatchedCandidateSelector",
                                       src   = cms.InputTag("trackProbes"),
                                       match = cms.InputTag("tkToMediumTightMuonsRPCMu"),
                                       )
...
process.passingTightMuonsRPCMu = cms.EDProducer("MatchedCandidateSelector",
                                       src   = cms.InputTag("trackProbes"),
                                       match = cms.InputTag("tkToTightMuonsRPCMu"),
                                       )
...
process.muonEffs = cms.EDAnalyzer("TagProbeFitTreeProducer",
    ...
    variables = cms.PSet(
        ...
        #dB  = cms.string("abs(dB)"),
        #nMuonHits       = cms.string("globalTrack().hitPattern().numberOfValidMuonHits()"),
        ...
        #nMatches        = cms.string("numberOfMatches"),
        #nStations       = cms.string("numberOfMatchedStations"),
        #nMatchesRPCMu   = cms.string("numberOfMatches('RPCHitAndTrackArbitration')"), 
        #nSstationsRPCMu = cms.string("numberOfMatchedStations('RPCHitAndTrackArbitration')"),
        #nLayersRPCMu    = cms.string("numberOfMatchedLayers('RPCHitAndTrackArbitration')"),
    ),
    ...
    flags = cms.PSet(
        ...
        PassingMediumTightMuonsRPCMu = cms.InputTag("passingMediumTightMuonsRPCMu"),
        PassingTightMuonsRPCMu = cms.InputTag("passingTightMuonsRPCMu"),
...
if MC_flag:
    process.tagAndProbe = cms.Path(
        ...
        * process.localreco * process.globalreco
        ...
        (process.muontrackingNoRPC+process.muonIdProducerSequenceNoRPC) * process.promptMuonsNoRPC *
        process.promptMuons *
        process.PassingHLT *
        process.tightMuons *
        ...
        process.trackProbes *
        process.MediumTightMuons * process.MediumTightMuonsNoRPC *
        process.MediumTightMuonsRPCMu *
        process.TightMuons * process.TightMuonsNoRPC *
        process.TightMuonsRPCMu *
        process.tkToMediumTightMuons * process.tkToMediumTightMuonsNoRPC *
        process.tkToMediumTightMuonsRPCMu *
        process.tkToTightMuons * process.tkToTightMuonsNoRPC *
        process.tkToTightMuonsRPCMu *
        process.passingMediumTightMuons * process.passingMediumTightMuonsNoRPC *
        process.passingMediumTightMuonsRPCMu *
        process.passingTightMuons * process.passingTightMuonsNoRPC *
        process.passingTightMuonsRPCMu *
